...
public class TambahLapInfoActivity extends AppCompatActivity implements
        OnMapReadyCallback,
        GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener,
        LocationListener {

    private static final String TAG = "TambahLapInfoActivity";
    ...
    @BindView(R.id.spinnerJenisLapInfo) Spinner spinnerJenisLapInfo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tambah_lap_info);
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        getSupportActionBar().setTitle("Buat Laporan Informasi");
    ...
    private void setupAttachmentPicker(){
        ...
        imagePicker = new ImagePicker(TambahLapInfoActivity.this);
        ...
        cameraImagePicker = new CameraImagePicker(TambahLapInfoActivity.this);
        ...
        videoPicker = new VideoPicker(TambahLapInfoActivity.this);
        videoPicker.setVideoPickerCallback(videoPickerCallback);
        videoPicker.shouldGenerateMetadata(true); // Default is true

        cameraVideoPicker = new CameraVideoPicker(TambahLapInfoActivity.this);
    ...
    public void submitForm(View view) {
        ...
        String jenisLapInfo = String.valueOf(spinnerJenisLapInfo.getSelectedItem());
        if(jenisLapInfo.equals("-- Pilih Jenis Lap. Info --")){
            Toast.makeText(this, "Anda belummemilih jenis lap. info..!!", Toast.LENGTH_SHORT).show();
            return;
        }

        String formJudul = etJudul.getText().toString();
        if (TextUtils.isEmpty(formJudul)) {
            Toast.makeText(this, "Anda belum memasukan judul lap. info..!!", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        builder.addFormDataPart("TrnLapInfo[jenis_id]", jenisMap.get(jenisLapInfo));
        builder.addFormDataPart("TrnLapInfo[judul]", formJudul);
        builder.addFormDataPart("TrnLapInfo[deskripsi]", formDeskripsi);
        builder.addFormDataPart("TrnLapInfo[accuracy]", String.valueOf(mLastLocation.getAccuracy()));
        builder.addFormDataPart("TrnLapInfo[lat]", String.valueOf(mLastLocation.getLatitude()));
        builder.addFormDataPart("TrnLapInfo[lng]", String.valueOf(mLastLocation.getLongitude()));
        builder.addFormDataPart("TrnLapInfo[provider]", mLastLocation.getProvider());
        builder.addFormDataPart("TrnLapInfo[speed]", String.valueOf(mLastLocation.getSpeed()));
        ...
        Call<LapInfo> call = service.createLapInfo(requestBody);
        call.enqueue(new Callback<LapInfo>() {
            @Override
            public void onResponse(Call<LapInfo> call, Response<LapInfo> response) {
                progress.dismiss();

                Gson gson = new Gson();

                if(response.isSuccessful()){
                    final LapInfo lapInfo = response.body();

                    AlertDialog.Builder builder = new AlertDialog.Builder(TambahLapInfoActivity.this);
                    builder.setTitle("Berhasil...!!");
                    builder.setMessage("Lap. Info berhasil dikirim. Apakah anda ingin manambah lap. info baru?");
                    ...
                    builder.setNegativeButton("Tidak",
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog,int which) {
                                    ...
                                    bundle.putParcelable(LapInfoDetailActivity.LAP_INFO_PARCEL, Parcels.wrap(lapInfo));
                                    ...
                                    Intent i = new Intent(TambahLapInfoActivity.this, LapInfoDetailActivity.class);
                                    i.putExtras(bundle);
                                    startActivity(i);

                                    TambahLapInfoActivity.this.finish();
                                }
                            });
                    builder.show();
                }else{
                    ...
                    try {
                        ...
                        if(response.code() == 422){
                            ...
                            Toast.makeText(TambahLapInfoActivity.this, errMsg, Toast.LENGTH_LONG).show();
                        }else{
                            Toast.makeText(TambahLapInfoActivity.this, response.message(), Toast.LENGTH_SHORT).show();
                        }
                    } catch (IOException e) {
                        Toast.makeText(TambahLapInfoActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                }
            }

            @Override
            public void onFailure(Call<LapInfo> call, Throwable t) {
                progress.dismiss();
                Toast.makeText(TambahLapInfoActivity.this, t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void resetForm(){
        ...
        spinnerJenisLapInfo.setSelection(0);
        etJudul.setText("");
        etDeskripsi.setText("");
    }
    ...
    private void getJenisLapInfo(){
        ...
        call.enqueue(new Callback<List<Jenis>>() {
            ...
            public void onResponse(Call<List<Jenis>> call, Response<List<Jenis>> response) {
                ...
                if(response.isSuccessful()){
                    jenisLapInfoList.addAll(response.body());

                    ArrayList<String> items = new ArrayList<String>();
                    items.add("-- Pilih Jenis Lap. Info --");
                    jenisMap.put("-- Pilih Jenis Lap. Info --", "");

                    for(Jenis jenis: jenisLapInfoList){
                        items.add(jenis.nama);
                        jenisMap.put(jenis.nama, String.valueOf(jenis.id));
                    }

                    ArrayAdapter<String> adapter = new ArrayAdapter<String>(TambahLapInfoActivity.this, R.layout.custom_spinner_item, items);
                    spinnerJenisLapInfo.setAdapter(adapter);
