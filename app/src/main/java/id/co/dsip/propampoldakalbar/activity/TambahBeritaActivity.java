...
public class TambahBeritaActivity extends AppCompatActivity implements
        OnMapReadyCallback,
        ...
        LocationListener
{
    ...
    private static final String TAG = "TambahBeritaActivity";
    ...
    private List<Jenis> jenisBeritaList = new ArrayList<Jenis>();
    ...
    @BindView(R.id.spinnerJenisBerita) Spinner spinnerJenisBerita;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Buat Berita");
    ...
    private void setupAttachmentPicker(){
        imagePickerCallback = new ImagePickerCallback() {
            ...
            public void onImagesChosen(List<ChosenImage> list) {
                ...
                for (ChosenImage img : list){
                    ...
                    Log.e("FOTO_SUCCESS", img.getThumbnailSmallPath());
                    Log.e("FOTO_SUCCESS", img.getMimeType());
                    Log.e("FOTO_SUCCESS", img.getFileExtensionFromMimeType());
                    Log.e("FOTO_SUCCESS", img.getType());
                }
        ...
        imagePicker = new ImagePicker(TambahBeritaActivity.this);
        ...
        cameraImagePicker = new CameraImagePicker(TambahBeritaActivity.this);
        ...
        videoPicker = new VideoPicker(TambahBeritaActivity.this);
        videoPicker.setVideoPickerCallback(videoPickerCallback);
        videoPicker.shouldGenerateMetadata(true); // Default is true

        cameraVideoPicker = new CameraVideoPicker(TambahBeritaActivity.this);
    ...
    public void submitForm(View view) {
        ...
        String jenisBerita = String.valueOf(spinnerJenisBerita.getSelectedItem());
        if(jenisBerita.equals("-- Pilih Jenis Berita --")){
            Toast.makeText(this, "Anda belummemilih jenis berita..!!", Toast.LENGTH_SHORT).show();
            return;
        }

        String formJudul = etJudul.getText().toString();
        if (TextUtils.isEmpty(formJudul)) {
            Toast.makeText(this, "Anda belum memasukan judul berita..!!", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        builder.addFormDataPart("TrnBerita[jenis_id]", jenisMap.get(jenisBerita));
        builder.addFormDataPart("TrnBerita[judul]", formJudul);
        builder.addFormDataPart("TrnBerita[deskripsi]", formDeskripsi);
        builder.addFormDataPart("TrnBerita[accuracy]", String.valueOf(mLastLocation.getAccuracy()));
        builder.addFormDataPart("TrnBerita[lat]", String.valueOf(mLastLocation.getLatitude()));
        builder.addFormDataPart("TrnBerita[lng]", String.valueOf(mLastLocation.getLongitude()));
        builder.addFormDataPart("TrnBerita[provider]", mLastLocation.getProvider());
        builder.addFormDataPart("TrnBerita[speed]", String.valueOf(mLastLocation.getSpeed()));
        ...
        Call<Berita> call = service.createBerita(requestBody);
        call.enqueue(new Callback<Berita>() {
            ...
            public void onResponse(Call<Berita> call, Response<Berita> response) {
                ...
                if(response.isSuccessful()){
                    final Berita berita = response.body();

                    AlertDialog.Builder builder = new AlertDialog.Builder(TambahBeritaActivity.this);
                    builder.setTitle("Berhasil...!!");
                    builder.setMessage("Berita berhasil dikirim. Apakah anda ingin manambah berita baru?");
                    ...
                    builder.setNegativeButton("Tidak",
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,int which) {
                                ...
                                bundle.putParcelable(BeritaDetailActivity.BERITA_PARCEL, Parcels.wrap(berita));
                                ...
                                Intent i = new Intent(TambahBeritaActivity.this, BeritaDetailActivity.class);
                                i.putExtras(bundle);
                                startActivity(i);

                                TambahBeritaActivity.this.finish();
                            }
                        });
                    builder.show();
                }else{
                    ...
                    try {
                        ...
                        if(response.code() == 422){
                            ...
                            Toast.makeText(TambahBeritaActivity.this, errMsg, Toast.LENGTH_LONG).show();
                        }else{
                            Toast.makeText(TambahBeritaActivity.this, response.message(), Toast.LENGTH_SHORT).show();
                        }
                    } catch (IOException e) {
    ...
    private void getJenisBerita(){
        tvLoading.setText("Memuat jenis berita...");

        OkHttpClient httpClient = new MyOkHttpInterceptor().getOkHttpClient();
        TransactionEndPoint service = ServiceGenerator.createService(TransactionEndPoint.class, httpClient);
        Call<List<Jenis>> call = service.getJenisBerita();
        call.enqueue(new Callback<List<Jenis>>() {
            ...
            public void onResponse(Call<List<Jenis>> call, Response<List<Jenis>> response) {
                ...
                if(response.isSuccessful()){
                    jenisBeritaList.addAll(response.body());

                    ArrayList<String> items = new ArrayList<String>();
                    items.add("-- Pilih Jenis Berita --");
                    jenisMap.put("-- Pilih Jenis Berita --", "");

                    for(Jenis jenis: jenisBeritaList){
                        items.add(jenis.nama);
                        jenisMap.put(jenis.nama, String.valueOf(jenis.id));
                    }

                    ArrayAdapter<String> adapter = new ArrayAdapter<String>(TambahBeritaActivity.this, R.layout.custom_spinner_item, items);
                    spinnerJenisBerita.setAdapter(adapter);
